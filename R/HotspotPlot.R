...
HotspotPlot <- function(CovidConfirmedCases, CovidDeaths, MAJCOMInput){
  ...
  tempCases = CovidConfirmedCases %>% 
    dplyr::select(-State, -stateFIPS) %>%
    reshape2::melt(id.var = c('CountyFIPS','County Name'), 
                   variable.name = 'date', 
                   value.name = "cumulative_cases") %>%
    dplyr::mutate(date = as.Date(stringr::str_replace(date, "X",""), format = "%m/%d/%y"), 
           `County Name` = as.character(`County Name`)) %>%
    dplyr::distinct(CountyFIPS,date,.keep_all = TRUE)
  
  CasesGrowth <- tempCases %>% 
    dplyr::group_by(CountyFIPS) %>% 
    dplyr::arrange(CountyFIPS, date) %>%
    dplyr::mutate(new_cases_1 = cumulative_cases - dplyr::lag(cumulative_cases, 1), 
                  new_cases_3_days = cumulative_cases - dplyr::lag(cumulative_cases,3),
                  new_cases_30_days = dplyr::lag(cumulative_cases,3) - dplyr::lag(cumulative_cases, 30),
  ...
  tempDeaths = CovidDeaths %>% 
    ...
    dplyr::mutate(date = as.Date(stringr::str_replace(date, "X",""), format = "%m/%d/%y"), 
                  `County Name` = as.character(`County Name`)) %>%
    dplyr::distinct(CountyFIPS,date,.keep_all = TRUE) %>% 
    dplyr::mutate(cumulative_deaths = ifelse(is.na(cumulative_deaths), 0, cumulative_deaths))
  
  # join Cases and deaths dataframes
  Growth = CasesGrowth %>% dplyr::left_join(tempDeaths, 
                                            by = c("CountyFIPS" = "CountyFIPS", "date" = "date", "County Name" = "County Name"))
  ...
  Growth = Growth %>% 
    dplyr::left_join(CountyInfo %>% dplyr::select(FIPS, Population), by = c("CountyFIPS" = "FIPS")) %>% 
  ...
  fix.fips <- function(column){
    column <- stringr::str_pad(column, width=5, side="left", pad="0")
    return(column)
  }
  # per capita calcs
  Growth = Growth %>% dplyr::mutate(new_cases_1_pp = new_cases_1*100000/Population, 
                                    new_cases_3_pp = new_cases_3_days*100000/Population,
                                    new_cases_30_pp = new_cases_30_days*100000/Population, 
                                    cases_pp = cumulative_cases*100000/Population,
                                    deaths_pp = cumulative_deaths*100000/Population) %>% 
    dplyr::ungroup() %>%
    dplyr::mutate(CountyFIPS = fix.fips(CountyFIPS))
  
  # Convert cimd dataframe to long format and filter to within 50 miles of base
  
  rownames(cimd) = CountyInfo[,3] 
  cimd_long <- cimd %>% tibble::rownames_to_column(var= "FIPS")
  cimd_long <- cimd_long %>% tidyr::gather(-c(FIPS), key = base, value = DistanceMiles) 
  Bases50 <- cimd_long %>% dplyr::filter(DistanceMiles <= 50) %>% dplyr::mutate(FIPS = fix.fips(FIPS))
  ...
  bases_radius <- Bases50 %>% dplyr::left_join(Growth, by = c("FIPS" = "CountyFIPS")) %>% dplyr::group_by(base, date) %>% 
    dplyr::summarise(cumulative_cases = sum(cumulative_cases), 
                     cases_pp = sum(cases_pp), 
                     new_cases_1_pp = sum(new_cases_1_pp), 
                     new_cases_3_pp = sum(new_cases_3_pp),
                     new_cases_30_pp = sum(new_cases_30_pp), 
                     cumulative_deaths = sum(cumulative_deaths), 
                     deaths_pp = sum(deaths_pp)) %>% 
    dplyr::mutate(case_growth = new_cases_3_pp/(new_cases_30_pp+new_cases_3_pp))
  
  bases_radius = bases_radius %>% 
    dplyr::left_join(AFBaseLocations %>% dplyr::select(Base, 'Major Command'), by = c("base" = "Base"))
  
  #this morning, cases were updated before deaths so I added in this code to pull the most current reported deaths date
  current_date = (bases_radius %>% 
                    dplyr::ungroup() %>% 
                    dplyr::filter(deaths_pp > 0) %>% 
  ...
  if (MAJCOMInput == "All"){
    ...
    bases_radius %>% dplyr::mutate(cases_30_trunc = pmin(new_cases_30_pp, 10000)) %>%  # had to truncate cases at 10000 before since Mcguire was goin nuts 
      dplyr::filter(new_cases_3_pp > 500, #filtering to show only bases with more than 200 cases per cap in last 3 days. gets cluttered if you include all
      ...
      ggrepel::geom_label_repel(aes(new_cases_3_pp, 
                                    case_growth, label = base),
                                fontface = 'bold', 
                                size = 3, 
                                fill = "white", 
                                color = "#00308f",
