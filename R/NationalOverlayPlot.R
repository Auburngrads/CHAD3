...
NationalOverlayPlot<-function(SocialDistance, DaysForecasted){
  ...
  Dataframe1<-IHME_Model %>% 
    dplyr::group_by(date) %>% 
    dplyr::summarise(allbed_mean = sum(allbed_mean))
  Dataframe2<-IHME_Model %>% 
    dplyr::group_by(date) %>% 
    dplyr::summarise(allbed_lower = sum(allbed_lower))
  Dataframe3<-IHME_Model %>% 
    dplyr::group_by(date) %>% 
    dplyr::summarise(allbed_upper = sum(allbed_upper))
