...
CHIMELocalPlot<-function(SocialDistance, 
    ...
    if (StatisticType == "Hospitalizations") {
        ...
        DailyProjectionsSub <- reshape2::melt(data.table::data.table(DailyData), id=c("ForecastDate"))
        TotalProjectionsSub <- reshape2::melt(data.table::data.table(TotalData), id=c("ForecastDate"))
